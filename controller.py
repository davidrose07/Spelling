...
class Controller(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.bad_words = []
        self.week = ""
        self.label_game_count.setText(f'Game Count: {Controller.game_count}')
        self.btn_speak.clicked.connect(lambda: (SCRAMBLE.play_mp3(Controller.correct_word), self.lineEdit_scramble.setFocus()))
        self.btn_validate.clicked.connect(lambda: self.validate(Controller.correct_word))
        self.lineEdit_scramble.setFocus()
    ...
    """ def add_letter(self,letter):
        self.lineEdit_scramble.setText(self.lineEdit_scramble.text() + letter) """
    ...
    def validate(self,correct_word) -> None:
        ...
        if correct_word != "":
            ...
            if phrase == "":
                ...
                self.msg_box.exec_()
                return
            if Controller.game_count == 2:
                if len(phrase) < len(correct_word) :
                    self.msg_box.setWindowTitle("Not Enough Letters")
                    self.msg_box.setText("Not enough letters!!")
                    self.msg_box.setStandardButtons(QMessageBox.Ok)
                    self.msg_box.exec_()
                    return 
            if len(phrase) > len(correct_word):
    ...
    def next_word(self) -> None:
        ...
        try:
            if len(Controller.spelling_list) > 0:
                ...
                time.sleep(1)
                SCRAMBLE.play_mp3(Controller.correct_word)
                self.lineEdit_scramble.setFocus()
